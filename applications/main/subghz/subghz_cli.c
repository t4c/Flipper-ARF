...
void subghz_cli_command_decode_raw(PipeSide* pipe, FuriString* args, void* context) {
    ...
    do {
        if(furi_string_size(args)) {
            if(!args_read_probably_quoted_string_and_trim(args, file_name)) { // [UNLEASHED_PORT]
                cli_print_usage(
...
static void subghz_cli_command_encrypt_keeloq(PipeSide* pipe, FuriString* args) {
    ...
    do {
        if(!args_read_probably_quoted_string_and_trim(args, source)) { // [UNLEASHED_PORT]
            subghz_cli_command_print_usage();
            break;
        }

        if(!args_read_probably_quoted_string_and_trim(args, destination)) { // [UNLEASHED_PORT]
            subghz_cli_command_print_usage();
            break;
        }
...
static void subghz_cli_command_chat(PipeSide* pipe, FuriString* args) {
    ...
    if(!furi_hal_subghz_is_tx_allowed(frequency)) {
        printf(
            ...
            frequency);
        // [UNLEASHED_PORT] UAF fix #829: release radio device before returning
        subghz_devices_deinit();
        subghz_cli_radio_device_power_off();
        return;
    }
