...
void ws_view_receiver_update_item(
    WSReceiver* ws_receiver, uint16_t idx, const char* name, uint8_t type) {
    furi_assert(ws_receiver);
    if(!name) return;
    ...
    with_view_model(
        ...
        WSReceiverModel * model,
        {
            if(idx < WSReceiverMenuItemArray_size(model->history->data)) {
                WSReceiverMenuItem* item_menu =
                    WSReceiverMenuItemArray_get(model->history->data, idx);
                if(item_menu && item_menu->item_str) {
                    furi_string_set(item_menu->item_str, name);
                    item_menu->type = type;
                }
            }
        },
...
void ws_view_receiver_draw(Canvas* canvas, WSReceiverModel* model) {
    ...
    case WSReceiverBarShowToUnlockPress:
        ...
        elements_multiline_text(canvas, 65, 26, "Unlock\nBACK x3");
    ...
    case WSReceiverBarShowUnlock:
        ...
        canvas_draw_str(canvas, 74, 62, "Unlock");
        break;
    default:
        canvas_draw_str(canvas, 44, 62, furi_string_get_cstr(model->frequency_str));
        canvas_draw_str(canvas, 79, 62, furi_string_get_cstr(model->preset_str));
