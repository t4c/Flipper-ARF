...
// THGR810-family IDs. Newentor/Unni/Liorque rebrands use rolling high nibble
// values (F024/F224/F824/FA24); F8B4 is an additional observed variant.
#define ID_THGR810_BASE 0xf024
#define ID_THGR810_ALT 0xf8b4

static bool oregon3_is_thgr810_family(uint16_t sensor_id) {
    return ((sensor_id & 0xF0FFU) == ID_THGR810_BASE) || sensor_id == ID_THGR810_ALT;
}
...
static uint8_t oregon3_sensor_id_var_bits(uint16_t sensor_id) {
    if(oregon3_is_thgr810_family(sensor_id)) {
        // nibbles: temp + hum + '0'
        return (4 + 2 + 1) * 4;
    }
...
static void ws_oregon3_decode_var_data(WSBlockGeneric* ws_b, uint16_t sensor_id, uint32_t data) {
    if(!oregon3_is_thgr810_family(sensor_id)) return;
