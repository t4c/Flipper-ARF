...
static bool ws_protocol_solight_te44_check(WSProtocolDecoderSolightTE44* instance) {
    const uint64_t payload =
        instance->decoder.decode_count_bit == 37 ? instance->decoder.decode_data >> 1 :
                                                   instance->decoder.decode_data;
    if(!payload) return false;
    if(((payload >> 8) & 0x0f) != 0x0f) return false; // const not 1111
    ...
    uint8_t msg_rubicson_crc[] = {
        ...
        payload >> 12,
        0xf0,
        (payload & 0xf0) | (payload & 0x0f)};

    return subghz_protocol_blocks_crc8(msg_rubicson_crc, 5, 0x31, 0x6c) == 0;
}
...
void ws_protocol_decoder_solight_te44_feed(void* context, bool level, uint32_t duration) {
    ...
    case SolightTE44DecoderStepReset:
        if((!level) &&
           (DURATION_DIFF(duration, ws_protocol_solight_te44_const.te_short * 8) <
            ws_protocol_solight_te44_const.te_delta * 4)) {
    ...
    case SolightTE44DecoderStepCheckDuration:
        ...
        if(DURATION_DIFF(instance->decoder.te_last, ws_protocol_solight_te44_const.te_short) >=
           ws_protocol_solight_te44_const.te_delta) {
            instance->decoder.parser_step = SolightTE44DecoderStepReset;
            break;
        }

        if(DURATION_DIFF(duration, ws_protocol_solight_te44_const.te_short * 8) <
           ws_protocol_solight_te44_const.te_delta * 4) {
            /* rtl_433 defines a row as 36 payload bits followed by one zero
               separator bit. Remove that separator before exposing the
               payload to the shared Weather Lab data model. The sync gap is
               also the delimiter that primes the next repeated row. */
            if((instance->decoder.decode_count_bit == 37) &&
               ((instance->decoder.decode_data & 1ULL) == 0) &&
               ws_protocol_solight_te44_check(instance)) {
                instance->generic.data = instance->decoder.decode_data >> 1;
                instance->generic.data_count_bit = 36;
            ...
            DURATION_DIFF(duration, ws_protocol_solight_te44_const.te_short * 2) <
            ws_protocol_solight_te44_const.te_delta * 2) {
