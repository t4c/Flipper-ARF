...
SubGhzProtocolStatus ws_protocol_decoder_emose601x_serialize(
    ...
    SubGhzProtocolStatus ret =
        ws_block_generic_serialize(&instance->generic, flipper_format, preset);
    if(ret != SubGhzProtocolStatusOk) return ret;

    uint32_t frame_bits = 120;
    if(!flipper_format_write_uint32(flipper_format, "FrameBits", &frame_bits, 1))
        return SubGhzProtocolStatusErrorParserOthers;
    if(!flipper_format_write_hex(
           flipper_format,
           "UpperData",
           (const uint8_t*)&instance->upper_decode_data,
           sizeof(instance->upper_decode_data)))
        return SubGhzProtocolStatusErrorParserOthers;
    if(!flipper_format_write_hex(
           flipper_format,
           "LowerData",
           (const uint8_t*)&instance->decoder.decode_data,
           sizeof(instance->decoder.decode_data)))
        return SubGhzProtocolStatusErrorParserOthers;
    return ret;
}
