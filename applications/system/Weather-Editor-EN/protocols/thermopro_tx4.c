...
static bool ws_protocol_thermopro_tx4_check_data(uint64_t normalized_data) {
    uint8_t type = normalized_data >> 33;
    return (type == THERMO_PRO_TX4_TYPE_1) || (type == THERMO_PRO_TX4_TYPE_2);
}
    ...
    case ThermoPRO_TX4DecoderStepCheckDuration:
        if(!level) {
            if(DURATION_DIFF(duration, ws_protocol_thermopro_tx4_const.te_short * 18) <
               ws_protocol_thermopro_tx4_const.te_delta * 10) {
                //Found sync
                /* This low delimiter also synchronizes the next row. */
                instance->decoder.parser_step = ThermoPRO_TX4DecoderStepSaveDuration;
                const uint8_t packet_bits = instance->decoder.decode_count_bit;
                if((packet_bits == 36U || packet_bits == 37U)) {
                    /* Normalize a current 36-bit packet to the editor's legacy
                       37-bit layout. rtl_433 also permits a trailing zero bit. */
                    instance->generic.data = instance->decoder.decode_data;
                    if(packet_bits == 36U) instance->generic.data <<= 1;
                    instance->generic.data_count_bit = packet_bits;
                    if(ws_protocol_thermopro_tx4_check_data(instance->generic.data)) {
...
SubGhzProtocolStatus
    ...
    if(instance->generic.data_count_bit != 36U && instance->generic.data_count_bit != 37U) {
