...
// [UNLEASHED_PORT] Longest duration the radio can hold a level for in a single step
...
struct SubGhzFileEncoderWorker {
    ...
    bool is_decoding; // [UNLEASHED_PORT]
    FuriString* str_data;
        ...
        while(strint_to_int32(str, &str, &duration, 10) == StrintParseNoError) {
            if((duration < -SUBGHZ_FILE_ENCODER_DURATION_MAX) ||
               (duration > SUBGHZ_FILE_ENCODER_DURATION_MAX)) {
                // [UNLEASHED_PORT] the radio cannot hold a level for that long, so on TX the
                // sample is replaced by a short pulse - when decoding, clamping keeps the long
...
static int32_t subghz_file_encoder_worker_thread(void* context) {
    ...
    // [UNLEASHED_PORT] nothing was put on the air when decoding, and is_async_complete_tx() never
    // turns true outside of a transmission, so this would spin until the worker is stopped
...
SubGhzFileEncoderWorker* subghz_file_encoder_worker_alloc(void) {
    ...
    instance->worker_stopping = true;
    instance->is_decoding = false; // [UNLEASHED_PORT]
...
bool subghz_file_encoder_worker_start(
    ...
    // [UNLEASHED_PORT] without a radio the samples go to a decoder, not on the air
    instance->device = radio_device_name ? subghz_devices_get_by_name(radio_device_name) : NULL;
    instance->is_decoding = (instance->device == NULL); // [UNLEASHED_PORT]
