 ...
 *
 * [UNLEASHED_PORT] Pass NULL as radio_device_name when the samples are fed to a decoder instead
 * of to the radio: the worker then neither waits for a transmission to complete nor rewrites
 * durations that are too long to be transmitted in one go. Replacing those with a short pulse
 * (which is what happens on TX) glues together bursts that are seconds apart, and the decoders
 * never resynchronise.
 *
 * @param instance Pointer to a SubGhzFileEncoderWorker instance
