...
static void subghz_protocol_encoder_came_atomo_get_upload(
    ...
    if(btn == 0x1) {
        ...
        btn = 0x6;
    } else if(btn == 0x5) { // [UNLEASHED_PORT]
        btn = 0x0C;
    } else if(btn == 0x6) { // [UNLEASHED_PORT]
        btn = 0x0E;
    }
...
void subghz_protocol_decoder_came_atomo_feed(void* context, bool level, uint32_t duration) {
    ...
    case CameAtomoDecoderStepReset:
        // There are two known options for the header: 72K us (TOP42R, TOP44R) or 12k us (found on TOP44RBN) / 19k us (TOPD4REN) [UNLEASHED_PORT]
        if((!level) && ((DURATION_DIFF(duration, subghz_protocol_came_atomo_const.te_long * 10) <
                        ...
                        (DURATION_DIFF(duration, subghz_protocol_came_atomo_const.te_long * 16) <
                         subghz_protocol_came_atomo_const.te_delta * 10) || // [UNLEASHED_PORT] TOPD4REN
...
static void subghz_protocol_came_atomo_remote_controller(SubGhzBlockGeneric* instance) {
    ...
    * */
    ...
    if(btn_decode == 0x0) {
        ...
        instance->btn = 0x4;
    } else if(btn_decode == 0x0C) { // [UNLEASHED_PORT]
        instance->btn = 0x5;
    } else if(btn_decode == 0x0E) { // [UNLEASHED_PORT]
        instance->btn = 0x6;
    }
    ...
    subghz_custom_btn_set_max(4); // [UNLEASHED_PORT] increased from 3 to 4 (RIGHT btn support)
}
...
static uint8_t subghz_protocol_came_atomo_get_btn_code(void) {
    ...
    if((custom_btn_id == SUBGHZ_CUSTOM_BTN_OK) && (original_btn_code != 0)) {
        ...
        case 0x4:
            btn = 0x1;
            break;
        case 0x5: // [UNLEASHED_PORT]
            btn = 0x1;
            break;
        case 0x6: // [UNLEASHED_PORT]
            btn = 0x1;
            break;
    ...
    } else if(custom_btn_id == SUBGHZ_CUSTOM_BTN_RIGHT) { // [UNLEASHED_PORT] new RIGHT btn mapping
        switch(original_btn_code) {
        case 0x1:
            btn = 0x5;
            break;
